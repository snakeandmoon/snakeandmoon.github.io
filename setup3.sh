...
#   14. CREATES/RENAMES BRANCH TO 'main' (NEW!)
#   15. PULLS or PUSHES content based on remote state
#   16. Shows final instructions
#
# =============================================================================
...
# ==========================================
# 9. LINK AND SYNC REMOTE REPOSITORY (UPDATED)
# ==========================================
...
link_and_sync_remote() {
    ...
    # ==========================================
    # NEW: ENSURE BRANCH IS NAMED 'main'
    # ==========================================
    ...
    if [ -z "$CURRENT_BRANCH" ]; then
        ...
        print_info "No branches exist. Creating 'main' branch..."
        git checkout -b main
        print_success "Created and switched to 'main' branch"
    elif [ "$CURRENT_BRANCH" = "master" ]; then
        # On master branch - rename to main
        print_info "Currently on 'master' branch. Renaming to 'main'..."
        git branch -M master main
        print_success "Renamed 'master' to 'main' and switched to 'main'"
    elif [ "$CURRENT_BRANCH" = "main" ]; then
        print_success "Already on 'main' branch"
    else
        # On some other branch - ask user
        print_warning "Currently on branch: $CURRENT_BRANCH"
        read -p "Do you want to rename this branch to 'main'? (y/n): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            git branch -M "$CURRENT_BRANCH" main
            print_success "Renamed '$CURRENT_BRANCH' to 'main'"
        else
            print_info "Keeping branch name: $CURRENT_BRANCH"
            MAIN_BRANCH="$CURRENT_BRANCH"
        fi
    fi

    # Set MAIN_BRANCH for later use
    MAIN_BRANCH=${MAIN_BRANCH:-main}

    # ==========================================
    # CHECK REMOTE AND SYNC
    # ==========================================
    ...
    if [ $? -ne 0 ]; then
        ...
        print_info "Creating initial commit and pushing to '$MAIN_BRANCH'..."
        ...
        if git status --porcelain | grep -q .; then
            ...
            print_warning "No changes to commit. Creating empty initial commit..."
            git commit --allow-empty -m "Initial commit"
            print_success "Created empty initial commit"
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "Try: git push -u origin $MAIN_BRANCH"
        fi
    else
        print_success "Successfully fetched from remote."

        # Check if remote has the branch
        REMOTE_HAS_BRANCH=$(git ls-remote origin "$MAIN_BRANCH" 2>/dev/null | wc -l)

        if [ "$REMOTE_HAS_BRANCH" -gt 0 ]; then
            print_info "Remote has branch '$MAIN_BRANCH' with content. Pulling..."
            ...
            if [ $? -eq 0 ]; then
                ...
                echo "Try: git pull origin $MAIN_BRANCH --allow-unrelated-histories"
            fi
        else
            print_info "Remote branch '$MAIN_BRANCH' doesn't exist. Preparing to push..."
